...
static int right_aligned_elements[] = {
  IDC_STATIC1, IDC_CAMZOOM_SLIDER, IDC_TOGGLEHAPTICS_BUTTON, IDC_RECORDCOMBO
};
#define NUM_RIGHT_ALIGNED_ELEMENTS 4
...
Crecord_playerDlg::Crecord_playerDlg(CWnd* pParent /*=NULL*/)
	: CDialog(Crecord_playerDlg::IDD, pParent) {
	//{{AFX_DATA_INIT(Crecord_playerDlg)
	m_selection_index = -1;
	//}}AFX_DATA_INIT
  initialized = 0;
...
void Crecord_playerDlg::DoDataExchange(CDataExchange* pDX) {
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(Crecord_playerDlg)
	DDX_Control(pDX, IDC_RECORDCOMBO, m_music_select);
	DDX_Control(pDX, IDC_CAMZOOM_SLIDER, m_camera_zoom_slider);	
	DDX_CBIndex(pDX, IDC_RECORDCOMBO, m_selection_index);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(Crecord_playerDlg, CDialog)
	//{{AFX_MSG_MAP(Crecord_playerDlg)
	ON_WM_QUERYDRAGICON()
	ON_WM_CLOSE()
  ON_WM_HSCROLL()
	ON_WM_MOUSEMOVE()
  ...
  ON_BN_CLICKED(IDC_TOGGLEHAPTICS_BUTTON, OnToggleHapticsButton)
	ON_CBN_SELCHANGE(IDC_RECORDCOMBO, OnSelchangeCombo1)
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
  ON_WM_SIZING()
  ON_WM_SIZE()
...
BOOL Crecord_playerDlg::OnInitDialog() {
...
HCURSOR Crecord_playerDlg::OnQueryDragIcon() {
	return (HCURSOR) m_hIcon;
}


void Crecord_playerDlg::OnClose() {
...
void Crecord_playerDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar) {
...
void Crecord_playerDlg::OnMouseMove(UINT nFlags, CPoint point) {
...
void Crecord_playerDlg::OnLButtonDown(UINT nFlags, CPoint point) {
...
void Crecord_playerDlg::OnLButtonUp(UINT nFlags, CPoint point) {
...
void Crecord_playerDlg::OnRButtonDown(UINT nFlags, CPoint point) {
...
void Crecord_playerDlg::OnRButtonUp(UINT nFlags, CPoint point) {
...
void Crecord_playerDlg::OnToggleHapticsButton() {
...
void Crecord_playerDlg::OnSizing(UINT fwSide, LPRECT pRect)
{
...
void Crecord_playerDlg::OnSize(UINT nType, int cx, int cy)
{
...
void Crecord_playerDlg::OnSelchangeCombo1() 
{
	UpdateData();

	if( m_selection_index < 0 ) return;

	g_main_app->load_record(m_selection_index);
